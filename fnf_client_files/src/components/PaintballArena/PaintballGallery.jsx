...
export default function PaintballGallery({ galleryImages, videos, metrics }) {
  ...
  // Use passed metrics
  const paintballMetrics = metrics;

  // Fallback images when API data is not available
  const fallbackImages = [

    {
      src: "https://cdn.acsdev.in/FNF/698c91d4c1666.jpg",
      alt: "Team strategy session before paintball battle",
      title: "Team Strategy",
      category: "Teamwork",
      likes: 189,
      type: "image"
    ...
    {
      src: "https://cdn.acsdev.in/FNF/69d3f3481df1a.jpg",
      alt: "Epic paintball battle in action",
      title: "Battle Action",
      category: "Combat",
      likes: 312,
      type: "image"
    ...
    {
      src: "https://cdn.acsdev.in/FNF/69d3f348dbfef.jpg",
      alt: "Professional paintball equipment and gear",
      title: "Pro Equipment",
      category: "Gear",
      likes: 156,
      type: "image"
    ...
    {
      src: "https://cdn.acsdev.in/FNF/69d3f3481df1a.jpg",
      alt: "Outdoor paintball arena with natural terrain",
  ...
  const categories = ["Battlefield", "Teamwork", "Combat", "Gear", "Celebration", "Facility"];
  const titles = [
    "Combat Zone",
    "Team Strategy",
    "Battle Action",
  ...
  const processedImages = React.useMemo(() => {
    if (galleryImages && galleryImages.length > 0) {
      return galleryImages.map((imageUrl, index) => ({
        src: imageUrl,
        alt: `Paintball combat image ${index + 1}`,
  ...
  const processedVideos = React.useMemo(() => {
    // console.log('Processing videos in PaintballGallery:', videos);

    if (videos && videos.length > 0) {
      return videos.map((video, index) => {
        const baseVideo = {
          alt: `Paintball combat video ${index + 1}`,
          title: video.label && video.label[0] ? video.label[0] : `Combat Video ${index + 1}`,
          category: categories[index % categories.length],
          likes: Math.floor(Math.random() * 200) + 150,
  ...
  const metricsData = React.useMemo(() => {
    const fallbackMetrics = [
      { value: "15", label: "Combat Markers", color: "red" },
      { value: "3", label: "Battle Zones", color: "amber" },
      { value: "12", label: "Hours Daily", color: "orange" },
      { value: "100%", label: "Combat Ready", color: "red" }
    ];
    ...
    metricsMapping.forEach((mapping, index) => {
      const value = paintballMetrics[mapping.field];
      const suffix = paintballMetrics[mapping.suffix] || '';
  ...
  return (
    <section id="PaintballGallery" className="relative bg-gradient-to-tr from-gray-900 via-gray-800 to-black text-white py-20 px-4 sm:px-6 md:px-10 lg:px-20 overflow-hidden">

      {/* New Camera-Inspired Background */}
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          ...
          <p className="text-gray-300 text-xl leading-relaxed max-w-4xl mx-auto">
            Explore our collection of thrilling paintball moments, from tactical battles to victory celebrations,
            captured at FNF Arena's premier outdoor combat facility.
          </p>
        </motion.div>
        ...
        >
          <div className="bg-gradient-to-br from-red-500/20 to-red-600/10 backdrop-blur-sm rounded-3xl p-12 shadow-2xl border border-red-500/30">
            <div className="flex items-center justify-center gap-3 mb-6">
              ...
              <h3 className="text-4xl font-bold font-sansitaOne text-white">Experience the Battle Live!</h3>
              <Zap className="w-8 h-8 text-red-400" />
            </div>
            <p className="text-gray-200 mb-8 max-w-2xl mx-auto text-lg">
              Ready to create your own combat memories? Book your paintball session today and become part of our warrior gallery.
            </p>
