...
const bowlingImages = {
  features: {
    lanes: "https://cdn.acsdev.in/FNF/69ce4ac2edac3.jpg",
    groups: "https://cdn.acsdev.in/FNF/69ce4ac2edac3.jpg",
    safety: "https://cdn.acsdev.in/FNF/69ce4ac2edac3.jpg"
  },
  backgrounds: {
    bowlingBall1: "https://cdn.acsdev.in/FNF/69ce4ac2edac3.jpg",
    bowlingBall2: "https://cdn.acsdev.in/FNF/69ce4ac2edac3.jpg",
    lanes: "https://cdn.acsdev.in/FNF/69ce4ac2edac3.jpg"
  }
};

export default function BowlingFeatures({ features, details, galleryImages }) {
  const navigate = useNavigate();

  // Default features when no API data is available
  const defaultFeatures = [
    {
      title: "Professional Lanes",
      description: "8 state-of-the-art bowling lanes with automatic scoring systems and premium equipment.",
      image: bowlingImages.features.lanes
    },
    {
      title: "Group Events",
      description: "Perfect for birthday parties, corporate events, and family gatherings with group packages.",
      image: bowlingImages.features.groups
    },
    {
      title: "Safety First",
      description: "Sanitized equipment, non-slip lanes, and safety protocols for worry-free fun.",
      image: bowlingImages.features.safety
    }
  ];
  ...
  const featureIcons = [
    {
      ...
      reactIcon: <GiBowlingStrike className="w-12 h-12" />,
      secondaryIcon: <FaBowlingBall className="w-6 h-6" />
    },
    {
  ...
  return (
    <section className="relative bg-[#1e2125] text-white py-16 px-4 sm:px-6 md:px-10 lg:px-20 overflow-hidden">
      ...
      {/* Enhanced Bowling Background Illustrations */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        {/* Animated Bowling Balls */}
        <motion.div
        ...
        >
          <div className="w-32 h-32 rounded-full border-4 border-white/20 flex items-center justify-center relative">
            <img
              src={bowlingImages.backgrounds.bowlingBall1}
              alt="Bowling ball"
              className="w-full h-full rounded-full object-cover opacity-30"
            />
            <div className="absolute inset-0 flex items-center justify-center">
              <FaBowlingBall className="w-16 h-16 text-white/40" />
            </div>
          </div>
        </motion.div>
        ...
        >
          <div className="w-24 h-24 rounded-full border-4 border-red-500/20 flex items-center justify-center relative">
            <img
              src={bowlingImages.backgrounds.bowlingBall2}
              alt="Bowling ball"
              className="w-full h-full rounded-full object-cover opacity-30"
            />
            <div className="absolute inset-0 flex items-center justify-center">
              <FaBowlingBall className="w-12 h-12 text-red-500/40" />
            </div>
          </div>
        </motion.div>

        {/* Animated Bowling Pins Formation */}
        <motion.div
        ...
        >
          <div className="grid grid-cols-4 gap-2">
            {[...Array(10)].map((_, i) => (
              ...
              >
                <GiBowlingPin className="w-6 h-6 text-white/15" />
              </motion.div>
            ))}
          </div>
        </motion.div>
        ...
        >
          <GiBowlingStrike className="w-24 h-24 text-red-500/30" />
        </motion.div>
        ...
        {/* Abstract Bowling Lane with Image */}
        <div className="absolute bottom-0 left-0 right-0 h-32 opacity-5">
          <img
            src={bowlingImages.backgrounds.lanes}
            alt="Bowling lanes"
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          ...
          <h2 className="text-white text-[28px] md:text-[32px] font-sansitaOne mb-2">
            World-Class Bowling Facilities
          </h2>
          <h3 className="text-red-500 font-extrabold text-[36px] md:text-[42px] mt-1 font-sansitaOne mb-6">
            Strike Zone Excellence
          </h3>
          <p className="text-white mt-4 text-sm md:text-base leading-relaxed max-w-3xl mx-auto">
            Experience bowling like never before with our premium lanes, professional equipment,
            and exceptional service that makes every game memorable.
          </p>
        </motion.div>
        ...
        >
          <div className="bg-[#606265] rounded-2xl p-8 shadow-md border border-gray-200 relative overflow-hidden">
            ...
            <div className="absolute inset-0 flex items-center justify-center opacity-5">
              <GiBowlingStrike className="w-32 h-32 text-red-500" />
            </div>

            <div className="relative z-10">
              <div className="flex items-center justify-center gap-3 mb-4">
                <GiBowlingPin className="w-8 h-8 text-red-500" />
                <h3 className="text-3xl font-bold font-sansitaOne text-white">Ready to Bowl?</h3>
                <GiBowlingPin className="w-8 h-8 text-red-500" />
              </div>
              <p className="text-gray-300 mb-6 max-w-2xl mx-auto text-lg">
                Book your lane now and experience the thrill of strikes and spares at FNF Arena's premium bowling facility.
              </p>
              <p className="text-white text-lg font-medium max-w-2xl mx-auto">
                Walk-in games are welcome—just drop in and enjoy a fun bowling session!
              </p>
            </div>
          </div>
        </motion.div>
