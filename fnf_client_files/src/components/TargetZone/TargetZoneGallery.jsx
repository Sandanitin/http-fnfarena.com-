...
export default function TargetZoneGallery({ galleryImages, videos, metrics }) {
  ...
  // Use passed metrics
  const targetZoneMetrics = metrics;

  // Fallback images when API data is not available
  const fallbackImages = [

    {
      src: "https://cdn.acsdev.in/FNF/69e9ea3a85448.jpg",
      alt: "Archer drawing bow with perfect form and technique",
      title: "Perfect Form",
      category: "Technique",
      likes: 189,
      type: "image"
    ...
    {
      src: "https://cdn.acsdev.in/FNF/69e9ea3b7ca95.jpg",
      alt: "Bullseye hit with arrow in center ring",
      title: "Bullseye Hit",
      category: "Accuracy",
      likes: 312,
      type: "image"
    ...
    {
      src: "https://cdn.acsdev.in/FNF/69e9ea3c25db0.jpg",
      alt: "Professional archery equipment and compound bows",
      title: "Pro Equipment",
      category: "Gear",
      likes: 156,
      type: "image"
    ...
    {
      src: "https://cdn.acsdev.in/FNF/69e9ea3b7ca95.jpg",
      alt: "Outdoor archery range with natural backdrop",
  ...
  const categories = ["Precision", "Technique", "Accuracy", "Gear", "Learning", "Facility"];
  const titles = [
    "Target Range",
    "Perfect Form",
    "Bullseye Hit",
  ...
  const processedVideos = React.useMemo(() => {
    // console.log('Processing videos in TargetZoneGallery:', videos);

    if (videos && videos.length > 0) {
      return videos.map((video, index) => {
        const baseVideo = {
          alt: `Target zone video ${index + 1}`,
          title: video.label && video.label[0] ? video.label[0] : `Target Video ${index + 1}`,
          category: categories[index % categories.length],
          likes: Math.floor(Math.random() * 200) + 150,
  ...
  const metricsData = React.useMemo(() => {
    const fallbackMetrics = [
      { value: "20", label: "Target Lanes", color: "red" },
      { value: "5", label: "Distance Ranges", color: "amber" },
      { value: "12", label: "Hours Daily", color: "orange" },
      { value: "100%", label: "Precision Ready", color: "red" }
    ];

    if (!targetZoneMetrics) {
    ...
    metricsMapping.forEach((mapping, index) => {
      const value = targetZoneMetrics[mapping.field];
      const suffix = targetZoneMetrics[mapping.suffix] || '';
  ...
  return (
    <section id="TargetZoneGallery" className="relative bg-gradient-to-tr from-gray-900 via-gray-800 to-black text-white py-20 px-4 sm:px-6 md:px-10 lg:px-20 overflow-hidden">

      {/* New Target-Inspired Background */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        ...
        <div className="absolute bottom-20 left-20 opacity-8">
          <svg width="150" height="120" viewBox="0 0 150 120" className="text-red-400/20">
            <path d="M20 60 L120 60 M110 50 L120 60 L110 70" stroke="currentColor" strokeWidth="3" fill="none"/>
            <circle cx="130" cy="60" r="8" fill="currentColor" opacity="0.5"/>
          </svg>
        </div>

        {/* Scattered Target Icons */}
        ...
        <div className="absolute top-2/3 right-1/6 opacity-5 text-red-400 text-2xl">🎪</div>
      </div>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          ...
          <p className="text-gray-300 text-xl leading-relaxed max-w-4xl mx-auto">
            Explore our collection of precision shooting moments, from perfect bullseyes to training sessions,
            captured at FNF Arena's premier target zone and archery facility.
          </p>
        </motion.div>
                  ...
                  {media.type === 'youtube' ? (
                    <>
                      ...
                      <div className="absolute inset-0 flex items-center justify-center bg-black/20 group-hover:bg-black/40 transition-all duration-300">
                        <div className="w-16 h-16 bg-gradient-to-r from-red-500 to-red-600 rounded-full flex items-center justify-center text-white shadow-xl group-hover:scale-110 transition-transform duration-300 relative">
                    ...
                    <>
                      ...
                      <div className="absolute inset-0 flex items-center justify-center bg-black/20 group-hover:bg-black/40 transition-all duration-300">
                        <div className="w-16 h-16 bg-gradient-to-r from-red-500 to-red-600 rounded-full flex items-center justify-center text-white shadow-xl group-hover:scale-110 transition-transform duration-300">
                          <Play className="w-8 h-8 ml-1" />
                        </div>
                      </div>
            ...
            <div className="relative h-[500px] md:h-[600px] rounded-3xl overflow-hidden shadow-2xl border-2 border-red-500/30">
              ...
              {filteredMedia[activeMedia]?.type !== 'youtube' && (
                <>
                  <button
                    onClick={prevMedia}
                    className="absolute left-6 top-1/2 -translate-y-1/2 w-12 h-12 bg-gradient-to-r from-red-500/80 to-red-600/80 backdrop-blur-sm hover:from-red-600/90 hover:to-red-700/90 text-white rounded-full flex items-center justify-center transition-all duration-200 border border-white/30"
                  ...
                  <button
                    onClick={nextMedia}
                    className="absolute right-6 top-1/2 -translate-y-1/2 w-12 h-12 bg-gradient-to-r from-red-500/80 to-red-600/80 backdrop-blur-sm hover:from-red-600/90 hover:to-red-700/90 text-white rounded-full flex items-center justify-center transition-all duration-200 border border-white/30"
              ...
              {filteredMedia[activeMedia]?.type === 'youtube' && (
                <>
                  <button
                    onClick={prevMedia}
                    className="absolute left-6 bottom-6 w-12 h-12 bg-gradient-to-r from-red-500/80 to-red-600/80 backdrop-blur-sm hover:from-red-600/90 hover:to-red-700/90 text-white rounded-full flex items-center justify-center transition-all duration-200 border border-white/30"
                    aria-label="Previous media"
                  >
                    <ChevronLeft className="w-6 h-6" />
                  </button>
                  ...
                  <button
                    onClick={nextMedia}
                    className="absolute right-6 bottom-6 w-12 h-12 bg-gradient-to-r from-red-500/80 to-red-600/80 backdrop-blur-sm hover:from-red-600/90 hover:to-red-700/90 text-white rounded-full flex items-center justify-center transition-all duration-200 border border-white/30"
                    aria-label="Next media"
                  >
                    <ChevronRight className="w-6 h-6" />
                  </button>
                ...
                <button
                  ...
                  className={`relative flex-shrink-0 w-24 h-18 rounded-xl overflow-hidden transition-all duration-300 border-2 ${
                    index === activeMedia
                      ...
                      : 'border-gray-600 opacity-70 hover:opacity-100 hover:border-red-400'
                  }`}
                >
                  {media.type === 'youtube' ? (
                    <>
        ...
        >
          <div className="bg-gradient-to-br from-red-500/20 to-red-600/10 backdrop-blur-sm rounded-3xl p-12 shadow-2xl border border-red-500/30">
            <div className="flex items-center justify-center gap-3 mb-6">
              ...
              <h3 className="text-4xl font-bold font-sansitaOne text-white">Experience Precision Live!</h3>
              <Zap className="w-8 h-8 text-red-400" />
            </div>
            <p className="text-gray-200 mb-8 max-w-2xl mx-auto text-lg">
              Ready to create your own precision memories? Book your target zone session today and become part of our marksman gallery.
            </p>
