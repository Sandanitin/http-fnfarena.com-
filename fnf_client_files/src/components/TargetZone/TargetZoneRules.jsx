...
export default function TargetZoneRules({ protocols }) {
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState('requirements');

  // Get Target Zone protocols data
  const targetZoneProtocols = protocols && protocols.length > 0 ? protocols[0] : null;

  // Map API data to rule categories
  const ruleCategories = React.useMemo(() => {
    if (!targetZoneProtocols) {
      ...
      return {
        requirements: {
          ...
          items: [
            ...
            { text: "Under 14 must be supervised by adults at all times during shooting sessions" },
            { text: "Valid ID may be required for age verification before range entry" },
            { text: "Height and physical fitness requirements apply for certain target distances" }
          ]
        },
        equipment: {
          ...
          items: [
            { text: "MANDATORY SAFETY GEAR: ARM GUARDS, FINGER TABS & PROTECTIVE EQUIPMENT MUST BE WORN" },
            { text: "No removal of safety equipment during active shooting sessions" },
            { text: "Report any damaged equipment immediately to range officer" },
            { text: "No modifications or tampering with bows or arrows allowed" }
          ]
        },
        etiquette: {
          icon: <Users className="w-6 h-6" />,
          title: "Range Etiquette & Conduct",
          color: "from-red-700/20 to-red-800/20",
          borderColor: "border-red-700/40",
          items: [
            { text: "RANGE ETIQUETTE: NO TALKING, MOVING, OR DISTRACTING OTHERS DURING SHOOTING" },
            { text: "RESPECT: MAINTAIN QUIET ENVIRONMENT; NO DISRUPTIVE BEHAVIOR OR INTERFERENCE" },
            { text: "NO CROSSING THE SHOOTING LINE WITHOUT PERMISSION OR RANGE OFFICER APPROVAL" },
            { text: "Wait for 'all clear' signal before retrieving arrows from targets" }
          ]
        },
        rules: {
          icon: <Target className="w-6 h-6" />,
          title: "Range Safety & Protocols",
          color: "from-red-800/20 to-red-900/20",
          borderColor: "border-red-800/40",
          items: [
            { text: "NO ENTRY TO RANGE WITHOUT INSTRUCTOR PERMISSION AND SAFETY BRIEFING" },
            { text: "FOLLOW ALL RANGE COMMANDS AND INSTRUCTOR DIRECTIONS IMMEDIATELY" },
            { text: "INSTRUCTOR DECISIONS ARE FINAL; UNSAFE BEHAVIOR MAY LEAD TO REMOVAL" },
            { text: "Always point arrows downrange and never draw bow without arrow nocked" }
          ]
        }
      };
    }
    ...
    return {
      requirements: {
        ...
        color: "from-red-500/20 to-red-600/20",
        borderColor: "border-red-500/40",
        items: targetZoneProtocols.requirements ? targetZoneProtocols.requirements.map(req => ({ text: req })) : []
      },
      equipment: {
        icon: <Shield className="w-6 h-6" />,
        title: "Safety Equipment",
        color: "from-red-600/20 to-red-700/20",
        borderColor: "border-red-600/40",
        items: targetZoneProtocols.equipment ? targetZoneProtocols.equipment.map(eq => ({ text: eq })) : []
      },
      etiquette: {
        icon: <Users className="w-6 h-6" />,
        title: "Range Etiquette & Conduct",
        color: "from-red-700/20 to-red-800/20",
        borderColor: "border-red-700/40",
        items: targetZoneProtocols.etiquette ? targetZoneProtocols.etiquette.map(et => ({ text: et })) : []
      },
      rules: {
        icon: <Target className="w-6 h-6" />,
        title: "Range Safety & Protocols",
        color: "from-red-800/20 to-red-900/20",
        borderColor: "border-red-800/40",
        items: targetZoneProtocols.rules ? targetZoneProtocols.rules.map(rule => ({ text: rule })) : []
      }
    };
  }, [targetZoneProtocols]);
  ...
  const quickTips = [
    ...
    { icon: "🤫", title: "Stay Quiet", desc: "Maintain range silence", color: "from-red-700/20 to-red-800/20" },
    { icon: "👨‍🏫", title: "Follow Instructor", desc: "Obey all commands", color: "from-red-800/20 to-red-900/20" }
  ];
  ...
  return (
    <section className="relative bg-gradient-to-tr from-gray-900 via-gray-800 to-black text-white py-20 px-4 sm:px-6 md:px-10 lg:px-20 overflow-hidden">
      ...
      {/* Target Zone Themed Background */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        {/* Target Rings Pattern */}
        <div className="absolute inset-0 opacity-5">
          <svg width="100%" height="100%" viewBox="0 0 1200 800" className="text-red-400/20">
            <defs>
              <pattern id="targetPattern" x="0" y="0" width="120" height="120" patternUnits="userSpaceOnUse">
                <circle cx="60" cy="60" r="50" fill="none" stroke="currentColor" strokeWidth="2" opacity="0.6"/>
                <circle cx="60" cy="60" r="35" fill="none" stroke="currentColor" strokeWidth="1.5" opacity="0.5"/>
                <circle cx="60" cy="60" r="20" fill="none" stroke="currentColor" strokeWidth="1" opacity="0.4"/>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          ...
          <p className="text-gray-300 text-xl leading-relaxed max-w-4xl mx-auto">
            Follow these essential target zone rules to ensure a safe and enjoyable archery experience for all participants.
            Your cooperation helps maintain a secure range environment for everyone at FNF Arena.
          </p>
        </motion.div>
          ...
          <div className={`bg-gradient-to-br ${ruleCategories[activeTab].color} backdrop-blur-sm rounded-3xl p-8 border ${ruleCategories[activeTab].borderColor} shadow-2xl`}>
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                ...
                <p className="text-gray-300">Essential guidelines for target zone safety</p>
              </div>
            </div>
              ...
              <div className="flex-grow">
                ...
                <div className="space-y-3 text-gray-200">
                  <p className="text-lg leading-relaxed">
                    <strong className="text-red-400">IMMEDIATE STOP:</strong> Raise your hand and call "RANGE OFFICER" if injured or equipment malfunction occurs.
                  </p>
                  <p className="leading-relaxed">
                    <strong className="text-red-400">RANGE OFFICERS:</strong> Trained safety personnel monitor all shooting activity.
                  ...
                  <p className="leading-relaxed">
                    ...
                    from range without refund. Repeat offenders may be banned from facility.
                  </p>
                </div>
                ...
                <div className="mt-6 p-4 bg-black/30 rounded-2xl border border-red-500/30">
                  ...
                  <p className="text-gray-300">Range Officer: <span className="text-white font-semibold">On-site at all times</span></p>
        ...
        >
          <div className="bg-gradient-to-br from-red-500/20 to-red-600/10 backdrop-blur-sm rounded-3xl p-8 border border-red-500/30">
            <div className="flex items-center justify-center gap-3 mb-4">
              ...
              <h3 className="text-3xl font-bold mb-4 font-sansitaOne text-white">Ready for Precision Training?</h3>
              <Flag className="w-8 h-8 text-red-400" />
            </div>
            <p className="text-gray-300 mb-6 max-w-2xl mx-auto text-lg">
              Now that you understand the rules, book your target zone session and experience the ultimate precision training!
            </p>
